...
**打分表 `/vote/sheet`**：原生表格；粘性表头 + 粘性姓名列；数值输入按该项 min/max 设 `min`/`max`/`step=1`；Tab 原生顺序连续录入，方向键/Enter 网格移动；非整数、越界、未填项标红并在提交前汇总提示；行数超 50 分页，避免上千输入框同屏。

**后台 `/admin`**：概览 5 秒轮询展示各票种发放/已用/剩余与部门提交进度；票种页改权重与一键生成；职工页 Excel/CSV 导入与手工维护；项点页按部门配 min/max；设置页配总开关与起止时间；结果页按部门筛选、排名与三件套导出。
...
| items 与部门不匹配 | 校验 employeeId/criterionId 均属该 departmentId，防越权写 |
| 部门无职工或无项点 | 投票页空白态说明，不报错；提交空 items 直接拒绝 |
| 权重合计不为 100 | 保存票种时拒绝并提示差额 |
| 某部门某票种零票 | 归一化自动排除，结果页标注实际参与计算的票种 |
